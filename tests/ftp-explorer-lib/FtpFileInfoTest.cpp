...
namespace ftp_explorer {
    ...
    TEST(FtpFileInfo, from_file_name__valid) {
        EXPECT_EQ(
            FtpFileInfo::from_filename("balance_12345678_20200709.txt"),
            (FtpFileInfo{"balance", 12345678u, date::year_month_day{date::year{2020}, date::month{7u}, date::day{9u}}}));
        EXPECT_EQ(
            FtpFileInfo::from_filename("action_14292387_20011231.txt"),
            (FtpFileInfo{"action", 14292387u, date::year_month_day{date::year{2001}, date::month{12u}, date::day{31u}}}));
    }
